...
uint32_t usb2514b_init() {
  GPIO_Init(
      _HUB_RST_N_PORT,
      &(GPIO_InitTypeDef){(1 << _HUB_RST_N_PIN), GPIO_Mode_OUT, GPIO_Speed_2MHz,
  ...
  GPIO_Init(
      _CFG_SEL1_PORT,
      &(GPIO_InitTypeDef){(1 << _CFG_SEL1_PIN), GPIO_Mode_OUT, GPIO_Speed_2MHz,
                          GPIO_OType_PP, GPIO_PuPd_NOPULL});
  // GPIO_Init(_SUSP_IND_PORT, &(GPIO_InitTypeDef){(1 << _SUSP_IND_PIN),
  // GPIO_Mode_OUT, GPIO_Speed_2MHz, GPIO_OType_PP, GPIO_PuPd_NOPULL});
  GPIO_Init(_SDA_PORT,
            &(GPIO_InitTypeDef){(1 << _SDA_PIN), GPIO_Mode_AF, GPIO_Speed_2MHz,
                                GPIO_OType_OD, GPIO_PuPd_NOPULL});
  GPIO_Init(_SCL_PORT,
            &(GPIO_InitTypeDef){(1 << _SCL_PIN), GPIO_Mode_AF, GPIO_Speed_2MHz,
                                GPIO_OType_OD, GPIO_PuPd_NOPULL});

  GPIO_Init(_OCS_N1_PORT, &(GPIO_InitTypeDef){(1 << _OCS_N1_PIN), GPIO_Mode_OUT,
                                              GPIO_Speed_2MHz, GPIO_OType_PP,
                                              GPIO_PuPd_NOPULL});
  GPIO_Init(_OCS_N2_PORT, &(GPIO_InitTypeDef){(1 << _OCS_N2_PIN), GPIO_Mode_OUT,
                                              GPIO_Speed_2MHz, GPIO_OType_PP,
                                              GPIO_PuPd_NOPULL});
  GPIO_Init(_OCS_N3_PORT, &(GPIO_InitTypeDef){(1 << _OCS_N3_PIN), GPIO_Mode_OUT,
                                              GPIO_Speed_2MHz, GPIO_OType_PP,
                                              GPIO_PuPd_NOPULL});
  ...
  GPIO_Init(_5V_EN1_PORT, &(GPIO_InitTypeDef){(1 << _5V_EN1_PIN), GPIO_Mode_OUT,
                                              GPIO_Speed_2MHz, GPIO_OType_PP,
                                              GPIO_PuPd_NOPULL});
  GPIO_Init(_5V_EN2_PORT, &(GPIO_InitTypeDef){(1 << _5V_EN2_PIN), GPIO_Mode_OUT,
                                              GPIO_Speed_2MHz, GPIO_OType_PP,
                                              GPIO_PuPd_NOPULL});
  GPIO_Init(_5V_EN3_PORT, &(GPIO_InitTypeDef){(1 << _5V_EN3_PIN), GPIO_Mode_OUT,
                                              GPIO_Speed_2MHz, GPIO_OType_PP,
                                              GPIO_PuPd_NOPULL});
  ...
  for (uint32_t byte = 0; byte < sizeof(hub_cfg); byte += 32) {
    ...
    rval =
        i2c(I2C1, (0x2C << 1), sizeof(cpy_buff), (uint8_t *)cpy_buff, 0, NULL);
