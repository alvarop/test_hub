...
void i2cSetup(uint32_t speed) {
  ...
  GPIO_PinAFConfig(_SDA_PORT, _SDA_PIN, GPIO_AF_1);
  GPIO_PinAFConfig(_SCL_PORT, _SCL_PIN, GPIO_AF_1);

  GPIO_Init(_SDA_PORT,
            &(GPIO_InitTypeDef){(1 << _SDA_PIN), GPIO_Mode_AF, GPIO_Speed_50MHz,
                                GPIO_OType_OD, GPIO_PuPd_NOPULL});

  GPIO_Init(_SCL_PORT,
            &(GPIO_InitTypeDef){(1 << _SCL_PIN), GPIO_Mode_AF, GPIO_Speed_50MHz,
                                GPIO_OType_OD, GPIO_PuPd_NOPULL});
  ...
  // i2cConfig.I2C_Mode = I2C_Mode_SMBusHost;
...
int32_t i2c(I2C_TypeDef *I2Cx, uint8_t addr, uint16_t wLen, uint8_t *wBuff,
            uint16_t rLen, uint8_t *rBuff) {
  int32_t rval = 0;
  uint32_t reg;
  uint32_t timeout;
  ...
  do {
    ...
    if (wLen > 0) {
      ...
      while (wLen--) {
        do {
          reg = I2Cx->ISR;
        } while (!(reg & (I2C_ISR_TXIS | I2C_ISR_NACKF)) && !i2cErr &&
                 (tickMs < timeout));
